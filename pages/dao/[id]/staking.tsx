import { CapsInfo, Header, Subheader } from "@components/creation/utilities/HeaderComponents";
...
const StakingActivity: React.FC = () => {
    const stakingActivities = activities.filter((i: IActivity) => i.category === 'Staking')
    return <Box>
    <Subheader title="Activity" />
    {stakingActivities.concat(stakingActivities).concat(stakingActivities).map((i: any, c: number) => {
        ...
        <Box sx={{ ml: "auto" }}>
            <Link href={`/dao/${id}/staking/manage`}>
                <Button variant="contained" size="small">
                    Manage Stake <img src={Coin.src} style={{ marginLeft: ".5rem", filter: themeContext.theme === LightTheme ? "invert(100%)" : '' }} />
                </Button>
            </Link>
