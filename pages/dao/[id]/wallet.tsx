...
const ActiveWallet: React.FC<{previous?: boolean}> = (props) => {
  ...
  return <Box sx={{
    ...
    <Box sx={{display: 'flex', alignItems: 'center'}}>
      <Avatar src={ImagePlaceholder.src} sx={{width: '2.5rem', height: '2.5rem'}}/>
      <Box sx={{ml: '1rem'}}>
        Nautilus
        <Box sx={{fontSize: '.9rem', color: 'text.light'}}>
          Connected at: {dateFormat(new Date(), 'mmm d, yyyy: h:ssTT')}
        </Box>
      </Box>
    </Box>
    <Box sx={{display: 'flex', alignItems: 'center', mt: '1rem'}}>
      ...
      <Box sx={{ml: 'auto'}}>
        <Chip
          icon={<Avatar src={PaideiaLogo.src} sx={{height: '1.5rem', width: '1.5rem'}}/>}
          label='56,759 PTK'
        />
      </Box>

    </Box>
    <Box sx={{display: 'flex', justifyContent: 'center', width: '100%', mt: '1rem', alignItems: 'center', flexDirection: 'column'}}>
      <Button onClick={() => setShow(!show)}>
        {show ? 'Hide' : 'Show'} other wallet addresses {show ? <KeyboardArrowUpIcon sx={{ml: '.3rem'}} color='primary'/> : <KeyboardArrowDownIcon sx={{ml: '.3rem'}} color='primary'/>}
      </Button>
      {show && <Box>
          {dAppWallet.addresses.filter((i: string) => i !== wallet).map((i: string, c: number) => <Box key={`other-wallet-addresses-key-${c}`} sx={{mt: '.5rem', mb: '.5rem'}}>
            {i}
            </Box>)}
      </Box>}
    </Box>
  ...
  return <Layout>
    <Box sx={{width: '100%', display: 'flex', alignItems: 'center'}}>
      <Header
        ...
        large
      />
      <Button sx={{ml: 'auto'}} variant='contained' onClick={handleClickOpen}>
        Change Wallet
        <AccountBalanceWalletIcon sx={{ml: ".5rem"}}/>
      </Button>
    </Box>
    <TabContext value={value}>
    <Box sx={{width: '100%', borderBottom: '1px solid', borderBottomColor: 'divider.main', mt: '1rem'}}>
