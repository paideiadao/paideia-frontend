...
const Reference: React.FC = () => {
    ...
    multiple
    id="tags-filled"
    isOptionEqualToValue={(option: any, temp: string) => value.references.indexOf(option.id) > -1}
    options={proposals || []}
    ...
    renderTags={(value: readonly string[], getTagProps) =>
      value.map((option: any, c: number) => {
          ...
          let val = temp[0]
        return <Chip color='primary' variant="filled" label={val.proposalName} {...getTagProps({ index: c })} key={'chip-tag-' + c} avatar={<Avatar sx={{fontSize: '.8rem'}}>
            
        </Avatar>}/>
      }
      )
    }
    onInputChange={(event: React.SyntheticEvent, _value: string, reason: string) => {
        console.log(event, _value)
    }}
    // @ts-ignore
    onChange={(event: React.SyntheticEvent, _value: any, reason: string, details?: string) => {
    ...
    renderOption={(props, option: any) => (
        ...
        <li {...props} onClick={() => {
            ...
            <Box sx={{ml: '.5rem', fontSize: '.9rem'}}>
                {option.proposalName}
                <Box sx={{fontSize: '.8rem', fontWeight: 400, color: 'text.light'}}>
                    ID: {option.id}
                </Box>
            </Box>
