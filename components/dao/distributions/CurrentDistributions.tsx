    ...
    return <Box sx={{width: '100%', mt: '.5rem', mb: '.5rem', p: '.5rem', display: 'flex', alignItems: 'center', backgroundColor: 'fileInput.outer', border: '1px solid', borderColor: 'divider.main', borderRadius: '.3rem'}}>
        <Box sx={{width: '26%'}}>
            ...
            <Box sx={{fontSize: '.8rem', color: 'text.light'}}>
                {dateFormat(props.startDate, 'mmmm d, yyyy')} - {dateFormat(props.endDate, 'mmmm d, yyyy')}
            </Box>
        </Box>
        <Box sx={{width: '22%'}}>
            <Chip
                variant='outlined'
                icon={props.type === 'Airdrop' ? <CellTowerIcon sx={{fontSize: '1.1rem', ml: '.5rem'}}/> : <AttachMoneyIcon sx={{fontSize: '1.1rem', ml: '.5rem'}}/>}
                color='primary'
                sx={{fontSize: '.7rem'}}
                label={props.type}
            />
        ...
        <Box sx={{width: '22%', display: 'flex', alignItems: 'center', fontSize: '.8rem', color:
                  props.status === "Active"
                    ? "primary.lightSuccess"
                    : props.status === 'Finished' ? 'error.main' : "tokenAlert.main",}}>
...
const CurrentDistributions: React.FC = () => {
    return <Box sx={{mt: '1rem'}}>
        ...
        {currentDistributions.map((i: IDistributionCard, c: number) => <DistributionCard {...i} key={`${c}-current-distribution-card`}/>)}
    </Box>
}
